...
def find_webapp_files
  FileList['src/main/webapp/**/*'].each do |src|
...
def find_coffee_files
  FileList["src/main/webapp/app/**/*.coffee"].each do |coffee|
...
#======================================================================
JADE_JS = "#{$target_directory}/app/views/jade.js"
file JADE_JS do
  $build_count += 1
  puts "coffeejade --amdout jade.js --amdrequire 'app/frameworks'"
  result = `cd #{$target_directory}/app/views && coffeejade --amdout jade.js --amdrequire 'frameworks' * 2>&1`
  if $?!=0
    touch JADE_JS
    raise result
  end
...
def find_jade_files
  FileList["src/main/webapp/app/views/**/*.jade"].each do |jade|
